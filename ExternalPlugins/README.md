# Installation, configuration and development of external plugins for Safeguard Secrets Broker for DevOps
...
Once installed, a plugin can be configured by clicking on the ```Edit Configuration``` link in the respective plugin card. This will cause Secrets Broker to display the ```Plugin Settings``` page.
...
* ***Plugin Instances*** - Multiple instances of a plugin can be created and configured to push credentials to different secrets vaults of the same type.
  * The ```New Instance``` button will create a new instance of the existing plugin with the option of copying the current plugin configuration to the new instance.
  * The previous and next buttons will switch the current configuration display between the multiple instances of a plugin.
  * The ```Delete Instance``` button will delete the current instance of a plugin.

* ***Delete Plugin*** - If the plugin and all of it's instances are no longer needed, the plugin and all instances can be removed from Secrets Broker by clicking on the ```Delete Plugin``` button.

* ***Save*** - To save the configuration for all instances of a plugin to the Secrets Broker database, click on the ```Save``` button.
